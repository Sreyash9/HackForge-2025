...
export function SafetyFeatures({ onNavigate }: SafetyFeaturesProps) {
  ...
  const getCurrentUser = (): { id: string; name?: string; phone?: string } | null => {
    try {
  ...
  const getStaticLandmarks = (lat: number, lng: number) => {
    ...
    if (user?.id) {
      try {
        ...
        if (data && Array.isArray(data)) {
          const numbers = data.map((c: any) => String(c.phone)).filter(Boolean);
