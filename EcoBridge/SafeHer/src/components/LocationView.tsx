...
export function LocationView() {
  ...
  const getGoaStaticPlaces = (lat: number, lng: number) => {
    // Static data for Goa area with calculated distances
    const staticPlaces = [
      {
    ...
    return staticPlaces
      .filter(place => parseFloat(place.distance) <= 5.0) // Only within 5km
  ...
  const getDangerZones = () => {
    // Define dangerous areas to avoid (these would typically come from user reports)
    return [
  ...
  const handlePlanSafeRoute = async () => {
    if (!userLocation) {
      setRouteError("Please allow location access to plan your route");
      return;
    }
    ...
    try {
      const route = await getSafeRoute(userLocation, destination);
      setSafeRoute(route);
    } catch (error) {
